...
define(["require", "exports", "dojo/query", "dojo/dom-attr", "dojo/NodeList-manipulate"], function (require, exports, dojoQuery, dojoAttr) {
    var _localeData = {};
     ...
     */
    function _applyLocaleToNode(node, locale) {
     ...
     */
    function _getLocaleString(localdef) {
        var bits = localdef.split(".");
        var root = _localeData;
        for (var b = 0; b < bits.length; b++) {
            if (root[bits[b]] == undefined) {
                return "";
            }
            root = root[bits[b]];
        }
    ...
    var Util = (function () {
         ...
         */
        Util.registerLocaleResource = function (localeData, resourceName) {
            _localeData[resourceName] = localeData;
        };
        /*
         * Helper Function for getting a String from a Dojo i18n NLS resource
         */
        Util.t = function (resourceName) {
         ...
         */
        Util.log = function (message, error) {
         ...
         */
        Util.error = function (message, error) {
            if ((error === undefined) || (error === null)) {
        ...
        */
        Util.debug = function (message, error) {
            if ((error === undefined) || (error === null)) {
        ...
        */
        Util.info = function (message, error) {
            if ((error === undefined) || (error === null)) {
         ...
         */
        Util.multiReplace = function (str, match, repl) {
            if (match === repl)
                return str;
            do {
        ...
        *  Pad a number with 0
        */
        Util.padDigits = function (mynumber, digits) {
            return Array(Math.max(digits - String(mynumber).length + 1, 0)).join("0") + mynumber;
        };
        /*
       *  Hot Key Implementation copied from jqery.hotkeys
       */
        Util.hotkey = function (selector, eventname, keycode, callfunction) {
            keycode = keycode.toLowerCase();
            var d = dojoQuery(selector).on(eventname, function (event) {
                if (this !== event.target && (/textarea|select/i.test(event.target.nodeName) ||
                    event.target.type === "text" || dojoAttr.get(event.target, 'contenteditable') == 'true')) {
                    return;
                }
                // Keypress represents characters, not special keys
                var special = event.type !== "keypress" && specialKeys[event.which], character = String.fromCharCode(event.which).toLowerCase(), key, modif = "", possible = {};
